import Link from "next/link";
import { MultimodalQueryBox } from "@/components/MultimodalQueryBox";
import { ThemeToggle } from "@/components/ThemeToggle";

export default function ImagePage() {
  return (
    <div className="min-h-screen pb-16 text-primary">
      <main className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-6 pb-12 pt-10 lg:px-10">
        <header className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          <Link href="/" className="flex items-center gap-3">
            <div className="glass-surface flex h-12 w-12 items-center justify-center text-xl font-semibold text-primary">
              V
            </div>
            <div>
              <p className="text-sm font-semibold tracking-[0.2em] text-tertiary">
                VERDICT AI
              </p>
              <h1 className="text-2xl font-semibold text-primary">
                Image Detection
              </h1>
            </div>
          </Link>
          <div className="flex flex-wrap items-center gap-3">
            <nav className="glass-surface flex flex-wrap items-center gap-3 px-4 py-2 text-xs text-secondary">
              <Link href="/text" className="rounded-full px-3 py-1 hover:text-primary">
                Text
              </Link>
              <Link
                href="/image"
                className="rounded-full bg-white/10 px-3 py-1 text-primary [data-theme='dark']:bg-white/10 [data-theme='dark']:text-white"
              >
                Image
              </Link>
              <Link href="/video" className="rounded-full px-3 py-1 hover:text-primary">
                Video
              </Link>
              <Link
                href="/community"
                className="rounded-full px-3 py-1 hover:text-primary"
              >
                Community
              </Link>
            </nav>
            <ThemeToggle />
          </div>
        </header>

        <section>
          <div className="glass-card flex flex-col gap-6 p-6 md:p-8">
            <p className="text-xs uppercase tracking-[0.3em] text-tertiary">
              Image Forensics
            </p>
            <h2 className="text-3xl font-semibold leading-tight text-primary md:text-4xl">
              Spot AI-generated images with advanced analysis
            </h2>
            <p className="text-sm text-secondary">
              Upload or paste an image URL to check if it was created by AI. We
              analyze compression patterns, metadata, and visual artifacts that
              AI generators leave behind.
            </p>
            <div className="flex flex-wrap gap-3 text-xs text-secondary">
              <span className="glass-surface px-3 py-2">Error Level Analysis</span>
              <span className="glass-surface px-3 py-2">EXIF metadata check</span>
              <span className="glass-surface px-3 py-2">FFT pattern detection</span>
            </div>
          </div>
        </section>

        <MultimodalQueryBox />

        <section className="mt-16">
          <div className="glass-card p-8 md:p-10">
            <h3 className="text-2xl font-semibold text-primary">
              How image detection works
            </h3>
            <div className="mt-6 grid gap-6 md:grid-cols-2">
              <div className="flex flex-col gap-3">
                <h4 className="text-lg font-semibold text-primary">
                  Compression analysis
                </h4>
                <p className="text-sm text-secondary">
                  AI-generated images often have uniform compression patterns
                  that differ from real camera photos. We highlight these
                  differences.
                </p>
              </div>
              <div className="flex flex-col gap-3">
                <h4 className="text-lg font-semibold text-primary">
                  Metadata inspection
                </h4>
                <p className="text-sm text-secondary">
                  Real photos contain camera and lens information. AI images
                  usually lack this or have suspicious metadata pointing to
                  generation software.
                </p>
              </div>
              <div className="flex flex-col gap-3">
                <h4 className="text-lg font-semibold text-primary">
                  Pattern detection
                </h4>
                <p className="text-sm text-secondary">
                  We use frequency analysis to spot repeating patterns and
                  artifacts that diffusion models often create, even in
                  high-quality images.
                </p>
              </div>
              <div className="flex flex-col gap-3">
                <h4 className="text-lg font-semibold text-primary">
                  Visual consistency
                </h4>
                <p className="text-sm text-secondary">
                  AI images sometimes have subtle inconsistencies in lighting,
                  shadows, or reflections that real photos don't have. We check
                  for these telltale signs.
                </p>
              </div>
            </div>
          </div>
        </section>
      </main>
    </div>
  );
}
